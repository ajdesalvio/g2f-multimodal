...
main <- function() {
  ...
  #### LOAD INPUTS ####
  VI <- fread(file.path(score_path, "FPC_Scores_BLUEs_AGDD_LOEO_Projected.csv")) %>% as.data.frame()
  ...
  W <- fread(file.path(score_path, "Weather_FPC_Scores_AGDD_LOEO_Projected.csv")) %>% as.data.frame()
  ...
  nFPCs <- suppressWarnings(as.integer(Sys.getenv("G2F_VI_NFPCS", unset = "7")))
