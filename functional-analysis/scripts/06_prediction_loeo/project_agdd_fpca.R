...
# This script uses AGDD as the time domain, NOT DAP!
# DAP column was deleted and replaced with AGDD to avoid perilous ctrl+F
# renaming throughout the script!
...
df.clim <- read.csv(file.path(data_path, 'EnvRtype_Weather_Data_Cleaned_V2.csv'))
...
# Weather columns (exclude identifiers and calendar/time columns).
w_vars <- names(df.clim)[match("T2M", names(df.clim)):match("PAR_TEMP", names(df.clim))]

# Inspect duplicate values for a few of the AGDDs
dups <- df.clim %>% select(all_of(c('Env', 'AGDD', 'T2M'))) %>%
  group_by(Env, AGDD) %>%
  summarize(n = dplyr::n(), .groups = "drop") %>%
  filter(n > 1L)
dups.inspect <- df.clim %>% filter(AGDD %in% dups$AGDD)

# On these days, the AGDD did not increase going from one calendar day to the next
# Split the weather data, average the duplicate values, then rejoin
df.clim.1 <- df.clim %>% filter(!AGDD %in% dups$AGDD)
df.clim.2 <- df.clim %>% filter(AGDD %in% dups$AGDD)

# Collapse duplicates
df.clim.2.avg <- df.clim.2 %>%
  group_by(Env, AGDD) %>%
  summarize(
    ## average every weather column
    across(all_of(w_vars), ~ mean(.x, na.rm = TRUE)),
    
    ## Keep one representative row for columns not used by the FPCA workflow.
    DAP       = first(DAP),              # or min(DAP) or max(DAP) if you prefer
    YYYYMMDD  = first(YYYYMMDD),         # ditto
    LON = first(LON),
    LAT = first(LAT),
    DOY = first(DOY),
...
# Rejoin the data frames
df.clim.clean <- bind_rows(df.clim.1, df.clim.2.avg) %>%
  arrange(Env, AGDD)

# Simpler data frame
dap.gdd <- df.clim.clean %>% select(Env, DAP, GDD, YYYYMMDD)
...
# Verify uniqueness of AGDDs
for (env_i in unique(dap.gdd$Env)) {
  temp <- dap.gdd %>% filter(Env == env_i)
  print(env_i)
  print(paste('Unique DAP:', length(unique(temp$DAP))))
  print(paste('Unique AGDD:', length(unique(temp$AGDD))))
}
...
VI <- fread(file.path(data_path, 'VI_BLUEs_G2F_2020_2021.csv')) %>%
  ...
  left_join(dap.gdd, by = c('Env', 'DAP')) %>%
  select(!DAP) %>%
  rename(DAP = AGDD)
# Weather
W <- df.clim.clean %>%
...
for (w_i in w_names) {
  ...
  W_i <- W %>%
    select(!c(GDD, YYYYMMDD)) %>%
    filter(Weather_Variable == w_i) %>%
    pivot_wider(
      names_from  = DAP,
      values_from = Value,
      names_glue  = "{w_i}_{DAP}",
      values_fill = NA_real_
    ) %>%
    as.data.frame()
  
  # Ensure proper ordering of DAPs for the MakeFPCAInputs function
  W_i.dap <- paste(w_i, w.dap.sorted, sep = '_')
...
fwrite(VI_scores_df, file.path(out_path, 'FPC_Scores_BLUEs_AGDD_LOEO_Projected.csv'))
...
fwrite(w_scores_df, file.path(out_path, 'Weather_FPC_Scores_AGDD_LOEO_Projected.csv'))
