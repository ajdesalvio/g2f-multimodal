...
run_cv_constant_kernels <- function(
  ...
  log_file <- if (isTRUE(write_outputs)) file.path(logs_dir, "DAP_CV_Constant_Kernels_V2.log") else NULL
  ...
  if (isTRUE(compute_eigs)) {
    ...
    log_message("Skipping eigendecomposition on constant kernels")
    eig_list_constant <- list()
  }
  ...
  summary_df <- data.frame(
    ...
    Eig_File = if (isTRUE(compute_eigs)) eig_file else NA_character_,
