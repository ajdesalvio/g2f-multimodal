...
build_weather_bundle <- function(
  ...
  eig_list <- if (isTRUE(compute_eigs)) {
    list(
...
run_cv_weather_setup <- function(
    ...
    weather_all_file = Sys.getenv("G2F_WEATHER_ALL_FILE", unset = "Weather_FPCA_Scores_ALL.csv"),
    weather_loeo_file = Sys.getenv("G2F_WEATHER_LOEO_FILE", unset = "Weather_FPC_Scores_DAP_LOEO_Projected.csv"),
  ...
  log_file <- if (isTRUE(write_outputs)) file.path(logs_dir, "DAP_CV_Weather_Setup_V3.log") else NULL
  ...
  if (!isTRUE(compute_eigs)) {
    log_message("Skipping eigendecomposition on weather-related kernels")
  }
  ...
  g2f_require_file(weather_loeo_path, "DAP LOEO weather score file")
