...
#SBATCH --job-name=DAP_CV_FPCA_Prep
...
COMBINED_SETUP_FILE="${G2F_JOBS_DIR}/DAP_CV_Setup_Jobs.txt"
COMBINED_PRED_FILE="${G2F_JOBS_DIR}/DAP_CV_Prediction_Jobs.txt"
LEGACY_JOBS_DIR="${G2F_CV_OUT_PATH}/job_tables"

if [[ ! -s "${COMBINED_SETUP_FILE}" ]]; then
  if [[ -s "${LEGACY_JOBS_DIR}/DAP_CV_Setup_Jobs.txt" ]]; then
    COMBINED_SETUP_FILE="${LEGACY_JOBS_DIR}/DAP_CV_Setup_Jobs.txt"
...
if [[ ! -s "${COMBINED_PRED_FILE}" ]]; then
  if [[ -s "${LEGACY_JOBS_DIR}/DAP_CV_Prediction_Jobs.txt" ]]; then
    COMBINED_PRED_FILE="${LEGACY_JOBS_DIR}/DAP_CV_Prediction_Jobs.txt"
