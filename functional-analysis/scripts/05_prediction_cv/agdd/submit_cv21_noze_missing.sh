...
#SBATCH --job-name=AGDD_CV21_noZe_Miss
...
PARAM_FILE="${G2F_JOBS_DIR}/AGDD_CV_Prediction_Jobs_CV_2_1_noZe_Missing_Only.txt"

if [[ ! -s "${PARAM_FILE}" ]]; then
  echo "No CV2/1 no-Ze restart jobs were found in ${PARAM_FILE}. Run the no-Ze audit first." >&2
  exit 0
fi
...
if [[ "${SPLIT_GROUP}" != "CV_2_1" ]]; then
  echo "Expected CV_2_1 in parameter file but found ${SPLIT_GROUP}" >&2
  exit 1
fi
