# Functional principal components for in-field soil-temperature trajectories.
...
trait <- "Soil Temperature [C]"
soil_config <- analysis$weather$station_soil_temperature

weather <- g2f_prepare_station_weather(paths, analysis)
weather <- g2f_apply_hampel_by_environment(
  weather,
  trait,
  environments = unlist(soil_config$hampel$environments, use.names = FALSE),
  k = soil_config$hampel$k,
  t0 = soil_config$hampel$t0
)
weather <- g2f_apply_dap_windows(
  weather,
  trait,
  soil_config$dap_windows
)
...
g2f_write_station_fpca(
  ...
  paths = paths,
  trait = trait,
  filename_stem = "Weather_FPCA_Soil_Temp",
  tall_value_name = "Soil.Temperature.C"
)
