...
class Aggregator:
    ...
    def __init__(
        ...
        # Optional explicit slug tag; falls back to the reductions join. Lets a
        # config node carry `name:` so `agg${aggregator.name}` resolves the same
        # way for both aggregators (see `name` property).
        self._name = name
        ...
        for r in reductions_list:
            if r not in self._FN_MAP:
                raise ValueError(
                    f"Invalid reduction '{r}'. "
                    f"Choose from {set(self._FN_MAP)}."
                )
        ...
        if "quantile" in self.reductions:
            if quantiles is None:
                raise ValueError(
                    "`quantiles` must be provided when "
                    "'quantile' is in reductions."
                )
    ...
    @property
    def name(self) -> str:
        """Short tag identifying the configured reductions.

        Uniform with :attr:`PMAAggregator.name` (e.g. ``mean``,
        ``mean+max`` vs ``pma``) so either aggregator exposes the same
        attribute. An explicit ``name`` arg overrides the default
        reductions join.
        """
        return self._name if self._name is not None else "+".join(self.reductions)

    @property
    def width_multiplier(self) -> int:
        """Output width as a multiple of the input feature dim ``D``.

        ``__call__`` concatenates one block of width ``D`` per reduction
        (``quantile`` contributes ``len(quantiles)`` blocks).
        """
        total = 0
        for r in self.reductions:
            if r == "quantile" and self.quantiles is not None:
    ...
    def _normalize_reduction_arg(
        ...
        if isinstance(reduction, str):
            ...
            raise TypeError(
                f"Expected str or list[str], got {type(reduction)}."
            )
    ...
    def _prepare_quantiles_tensor(
        ...
        if isinstance(quantiles, torch.Tensor):
            ...
            raise TypeError(
                f"Expected float/list/torch.Tensor, got {type(quantiles)}."
            )
        ...
        if q_unique.numel() < q.numel():
            warnings.warn(
                "Duplicate quantile levels removed. "
                f"Using: {q_unique.tolist()!r}"
            )
        return q_unique
    ...
    def _nanmean(self, x: torch.Tensor) -> torch.Tensor:
        # A fully-masked set (every element NaN along `dim`) makes
        # torch.nanmean return NaN; map that to 0.0 so an all-padding set
        # pools to a finite zero vector instead of poisoning the batch with
        # NaN. A partially-masked set still averages over its unmasked
        # elements only (torch.nanmean ignores the NaNs), so this only
        # touches the fully-masked rows.
        out = torch.nanmean(x, dim=self.dim, keepdim=self.keepdim)
        return torch.where(torch.isnan(out), torch.zeros_like(out), out)

    def _nanquantile(
        self, x: torch.Tensor, q: float | torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        if isinstance(q, torch.Tensor):
            if q.ndim != 1:
                raise ValueError(
                    f"Tensor of quantiles must be 1D, got {q.ndim}D."
                )
        ...
        raise TypeError(
            f"Expected float or 1D torch.Tensor for 'q', got {type(q)}."
        )
...
class PMAAggregator(nn.Module):
    ...
    convention, mirroring ``TransformerSetEncoder.padding_mask_to_attention_mask``.
    ...
    Args:
        ...
        seed_init_scale: Std-dev of the N(0, σ²) init for seed vectors.
        p_dropout: Attention dropout.
        name: Short identifying tag (default ``"pma"``).
    """

    @property
    def width_multiplier(self) -> int:
        """Output width as a multiple of ``embed_dim``.

        ``forward`` returns ``num_seeds`` flattened seed outputs, i.e.
        ``(B, num_seeds * embed_dim)``. Uniform with
        :attr:`Aggregator.width_multiplier` so either aggregator exposes
        the same attribute.
        """
    ...
    def __init__(
        ...
        super().__init__()
        # Short identifying tag; a real kwarg so it can sit on the
        # config node (uniform with `Aggregator.name`).
        self.name = name
        if head_dim is None:
        ...
        # One shared seed tensor across the batch; broadcast at forward time.
        # (see `width_multiplier` below — output is num_seeds blocks of embed_dim)
    ...
    def forward(
        ...
        all_masked: torch.Tensor | None = None
        if mask is not None:
            if mask.dtype != torch.bool:
                raise TypeError(
            ...
            if mask.shape != x.shape[:-1]:
                raise ValueError(
                    f"mask shape {tuple(mask.shape)} must match x.shape[:-1] "
                    f"= {tuple(x.shape[:-1])}."
                )
            # Aggregator: True = padding. SDPA: True = attend. Invert + add
            # query dim → broadcast over all seed queries.
            attn_mask = (~mask).unsqueeze(1)
            # A fully-masked set (every element padding) leaves an all-False
            # attend row → SDPA softmax over an all -inf row is NaN, which then
            # poisons the whole batch (and its gradients). Let those rows attend
            # to everything (finite garbage) so SDPA stays well-defined, then
            # zero their pooled output below — mirroring Aggregator._nanmean's
            # "all-masked → 0" convention so an all-padding set pools to a
            # finite zero vector instead of NaN.
            all_masked = mask.all(dim=-1)  # (B,)
            if all_masked.any():
                attn_mask = attn_mask | all_masked.view(-1, 1, 1)
        else:
            attn_mask = None

        out = self.attention(seeds, x, x, mask=attn_mask)  # (B, k, D)

        if all_masked is not None and all_masked.any():
            # Overwrite the fully-masked sets' pooled outputs with zeros. The
            # masked_fill also blocks gradient flow through those rows' (garbage)
            # attention, so no NaN reaches the seeds/projection grads.
    ...
    # Allow the encoder's `self.aggregator(x, mask=...)` call site to work
    # with both positional and keyword mask passing. nn.Module.__call__
    # already delegates to forward(), so no override needed here; the
    # TransformerSetEncoder line `self.aggregator(latent, mask=mask)` works.
