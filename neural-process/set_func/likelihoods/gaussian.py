...
from set_func.predictions import GaussianPrediction
...
class HeteroscedasticNormalLikelihood(BaseLikelihood):
    ...
    location_transform : TransformConfig, optional
        ...
        - list: sequence of transformations applied in order, e.g.
    ...
    def forward(self, x: torch.Tensor) -> GaussianPrediction:
        """
        Create a Normal prediction with input-dependent mean and standard deviation.

        Parameters
        ----------
        x : torch.Tensor
            Tensor containing both the mean and the raw (pre-transform)
            scale parameter concatenated along the last dimension; the
            scale half goes through ``scale_transform`` (softplus by
            default) to become the standard deviation

        Returns
        -------
        GaussianPrediction
            Normal prediction with input-dependent mean and standard deviation
        """
        if x.shape[-1] % 2 != 0:
        ...
        return GaussianPrediction(Normal(loc, scale))
...
class HomoscedasticNormalLikelihood(BaseLikelihood):
    ...
    def forward(self, x: torch.Tensor) -> GaussianPrediction:
        """
        Create a Normal prediction with mean x and constant standard deviation.
        ...
        -------
        GaussianPrediction
            Normal prediction with mean x and constant standard deviation
        """
