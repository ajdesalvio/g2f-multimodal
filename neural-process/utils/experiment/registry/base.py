...
class BaseWrapperRegistry(Generic[T]):
    ...
    def __init__(self):
        ...
        self._class_registry: dict[
            tuple[Type[nn.Module], Type[BaseBatch]], T
        ] = {}
        # Registry for instance-based wrappers (for specific model instances)
        self._instance_registry: dict[int, T] = {}
        # Cache for class hierarchy lookups, keyed on (model_type, batch_type).
        # Keying on types (not instances) is required because batch objects such
        # as G2FBatch are unhashable, and is also correct: the search only ever
        # depends on the model/batch *classes*.
    ...
    def register_class(
        ...
        model_types = (
            model_cls if isinstance(model_cls, tuple) else (model_cls,)
        )
        batch_types = (
            batch_cls if isinstance(batch_cls, tuple) else (batch_cls,)
        )
    ...
    def get_wrapper(self, model: nn.Module, batch: BaseBatch) -> T | None:
        ...
        # Use cached search for class hierarchy matching (keyed on types)
        return self._cache(model_type, batch_type)

    def _find_best_match_uncached(
        self, model_type: Type[nn.Module], batch_type: Type[BaseBatch]
    ) -> T | None:
        ...
        for reg_model_type, _ in self._class_registry:
            if inspect.isclass(reg_model_type) and issubclass(
        ...
        for _, reg_batch_type in self._class_registry:
            if inspect.isclass(reg_batch_type) and issubclass(
                batch_type, reg_batch_type
            ):
